...
const createOrderFromCheckoutSession = async (session) => {
  ...
  try {
    ...
    if (cartError || !cartItems || cartItems.length === 0) {
      console.error('❌ Cart error or empty:', { cartError, itemsFound: cartItems?.length });
  ...
  try {
    ...
    switch (event.type) {
      case 'checkout.session.completed':
        ...
        const { data: existingCheckoutOrder, error: checkError } = await supabaseAdmin
          .from('pedidos')
          .select('id')
          .eq('stripe_checkout_session_id', session.id)
          .single();

        console.log('🔍 Checking existing order:', { existingCheckoutOrder, checkError });

        if (!existingCheckoutOrder) {
          try {
            console.log('🏗️ Creating new order from checkout session...');
            const order = await createOrderFromCheckoutSession(session);
            console.log(`✅ Order ${order.id} created successfully from checkout session ${session.id}`);
          } catch (orderError) {
            console.error('❌ Failed to create order from checkout session:', orderError);
            console.error('❌ Full error stack:', orderError.stack);
            // Log the error but don't fail the webhook
          }
        } else {
          console.log(`⚠️ Order already exists for checkout session ${session.id}`);
        }
        break;
